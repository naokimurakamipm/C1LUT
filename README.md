...
既定では ICC CLUT は 33³ (Capture One 純正プロファイルと同じ仕様) に一様グリッドで焼き込みます。3 つの精度オプションがあります (**GUI の詳細設定タブでは既定でON** — すべて「改善した場合のみ適用」のガード付き。CLI ではフラグで明示指定):
...
ダークテーマ (チャコール + オレンジのアクセント、`conelut/theme.py`) で、タイトルバーまで統一色、高 DPI (200% など) でも滲まず描画されます。スクリーンショットは `python tools/shot_ui.py` で再生成できます。
